...
class DRBDataset_no_patch(data.Dataset):
    def __init__(self, opt):
        super(DRBDataset_no_patch, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        # change color space if necessary
        if self.opt['color']:
            img_LR = util.channel_convert(C, self.opt['color'], [img_LR])[0]  # TO DO during val no definition
