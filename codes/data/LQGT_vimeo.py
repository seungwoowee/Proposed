...
class LQGTDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.folders = [line.rstrip() for line in open(os.path.join(self.opt['dataroot_GT'], self.opt['file_list']))]
        gt_folders = [os.path.join(self.opt['dataroot_GT'], x) for x in self.folders]

        self.paths_GT = []
        for k in range(len(gt_folders)):
            for dirpath, _, fnames in sorted(os.walk(gt_folders[k])):
                for fname in sorted(fnames):
                    if is_image_file(fname):
                        img_path = os.path.join(dirpath, fname)
                        self.paths_GT.append(img_path)

        folders = [line.rstrip() for line in open(os.path.join(self.opt['dataroot_LQ'], self.opt['file_list']))]
        lr_folders = [os.path.join(self.opt['dataroot_LQ'], x) for x in folders]
        self.paths_LQ = []
        for k in range(len(lr_folders)):
            for dirpath, _, fnames in sorted(os.walk(lr_folders[k])):
                for fname in sorted(fnames):
                    if is_image_file(fname):
                        img_path = os.path.join(dirpath, fname)
                        self.paths_LQ.append(img_path)
