...
class DRBNet3(nn.Module):  # no warp model
    def __init__(self, rgb_range, scale):
...
class DRBNet9_3(nn.Module):
    def __init__(self, rgb_range, scale):
        super(DRBNet9_3, self).__init__()
        ...
        module_DRB = [
            module_util.DRBModule9_3(inout_ch=unit_feat, ch_reduction_ratio=ch_reduction_ratio, bias=use_bias,
                                   act=use_act,
                                   block_n=k + 1)
            for k in range(block_n)]
        ...
        self.HRconv = nn.Conv2d(in_channels=unit_feat + unit_feat, out_channels=unit_feat, kernel_size=3,
                                stride=1, padding=1,
                                bias=use_bias)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.conv_out = nn.Conv2d(in_channels=unit_feat, out_channels=scale ** 2 * 3, kernel_size=3, stride=1,
...
class DRBtest(nn.Module):
    def __init__(self, rgb_range, scale):
        super(DRBtest, self).__init__()
